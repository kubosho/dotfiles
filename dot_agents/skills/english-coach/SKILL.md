...
Records live in the Obsidian vault at `$XDG_DATA_HOME/obsidian/personal`. At session end, append the situation, the phrases the user can now use, and the final draft as a blockquote to `90_Journal/English_phrasebook.md`. Write each learning that generalizes beyond this session's draft as its own note in `01_Temporary/`, shaped per the vault README's atomicity rule, with `createdAt`/`updatedAt` frontmatter and a `[[English_phrasebook]]` link. At session start, read the phrasebook and the titles of notes that link to `[[English_phrasebook]]`, then prompt recall instead of pasting.
