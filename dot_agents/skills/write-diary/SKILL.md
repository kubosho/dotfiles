...
3. Append one entry section that summarizes the session as a whole. Organize it around the main points of the work rather than the order of requests. Split into multiple entries only when the session covered unrelated pieces of work that do not fit under one title, and in that case fill each field from that entry's own context.
