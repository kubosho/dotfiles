...
- 3回目以降の返答の先頭に `**n/t**` の形式で単独の行を置く。**n** は解消済みの枝の数、**t** はその時点で見えている枝の総数の見積もりを指す。2回目までの返答は手がかりが少なく見積もりが参考にならないため、`n/t` を含めない。
- n と t は、枝が解消したときと新しい枝が見えたときにだけ動く。それ以外の返答でも `**n/t**` の行は前回の値のまま表示を続ける。t が増えた場合も増えたことは特に触れない。
